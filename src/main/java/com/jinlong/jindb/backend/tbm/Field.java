...
import com.jinlong.jindb.backend.im.BPlusTree;
import com.jinlong.jindb.backend.parser.statement.SingleExpression;
import com.jinlong.jindb.backend.tm.TransactionManagerImpl;
import com.jinlong.jindb.backend.utils.Panic;
import com.jinlong.jindb.backend.utils.ParseStringRes;
...
/**
 * field 表示字段信息
 * 二进制格式为：[FieldName][TypeName][IndexUid]
 * 如果field无索引，IndexUid为 0
 *
 * @Author zjl
 * @Date 2024/6/18
 */
public class Field {
    ...
    public static Field loadField(Table table, long uid) {
        byte[] raw = null;
        try {
            raw = ((TableManagerImpl) table.tableManager).versionManager.read(TransactionManagerImpl.SUPER_XID, uid);
        } catch (Exception e) {
            Panic.panic(e);
        }
        assert raw != null;
    ...
    private Field parseSelf(byte[] raw) {
        ...
        if (index != 0) {
            try {
                bPlusTree = BPlusTree.load(index, ((TableManagerImpl) table.tableManager).dataManager);
            } catch (Exception e) {
                Panic.panic(e);
            }
        }
    ...
    public static Field createField(Table table, long xid, String fieldName, String fieldType, boolean indexed) throws Exception {
        ...
        if (indexed) {
            long index = BPlusTree.create(((TableManagerImpl) table.tableManager).dataManager);
            BPlusTree bPlusTree = BPlusTree.load(index, ((TableManagerImpl) table.tableManager).dataManager);
            field.index = index;
            field.bPlusTree = bPlusTree;
        }
    ...
    private void persistSelf(long xid) throws Exception {
        ...
        this.uid = ((TableManagerImpl) table.tableManager).versionManager.insert(xid, Bytes.concat(nameRaw, typeRaw, indexRaw));
    }

    private static void typeCheck(String fieldType) throws Exception {
        if (!"int32".equals(fieldType) && !"int64".equals(fieldType) && !"string".equals(fieldType)) {
            throw ErrorConstants.InvalidFieldException;
        }
    }
