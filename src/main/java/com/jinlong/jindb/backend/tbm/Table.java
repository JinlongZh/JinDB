...
public class Table {
    ...
    public static Table loadTable(TableManager tbm, long uid) {
        byte[] raw = null;
        try {
            raw = ((TableManagerImpl) tbm).versionManager.read(TransactionManagerImpl.SUPER_XID, uid);
        } catch (Exception e) {
            Panic.panic(e);
        }
        assert raw != null;
    ...
    private Table persistSelf(long xid) throws Exception {
        ...
        uid = ((TableManagerImpl) tableManager).versionManager.insert(xid, Bytes.concat(nameRaw, nextRaw, fieldRaw));
        return this;
    }
    ...
    public int delete(long xid, Delete delete) throws Exception {
        ...
        int count = 0;
        for (Long uid : uids) {
            if (((TableManagerImpl) tableManager).versionManager.delete(xid, uid)) {
                count++;
            }
        }
        return count;
    ...
    public int update(long xid, Update update) throws Exception {
        ...
        if (fd == null) {
            throw ErrorConstants.FieldNotFoundException;
        }
        Object value = fd.string2Value(update.value);
        int count = 0;
        for (Long uid : uids) {
            byte[] raw = ((TableManagerImpl) tableManager).versionManager.read(xid, uid);
            if (raw == null) continue;

            ((TableManagerImpl) tableManager).versionManager.delete(xid, uid);

            Map<String, Object> entry = parseEntry(raw);
            entry.put(fd.fieldName, value);
            raw = entry2Raw(entry);
            long uuid = ((TableManagerImpl) tableManager).versionManager.insert(xid, raw);
    ...
    public void insert(long xid, Insert insert) throws Exception {
        ...
        long uid = ((TableManagerImpl) tableManager).versionManager.insert(xid, raw);
        for (Field field : fields) {
            if (field.isIndexed()) {
    ...
    private Map<String, Object> string2Entry(String[] values) throws Exception {
        if (values.length != fields.size()) {
            throw ErrorConstants.InvalidValuesException;
        }
        ...
        if (where == null) {
            ...
            for (Field field : fields) {
                if (field.fieldName.equals(where.singleExp1.field)) {
                    if (!field.isIndexed()) {
                        throw ErrorConstants.FieldNotIndexedException;
                    }
                    fd = field;
                    break;
                }
    ...
    private CalWhereRes calWhere(Field fd, Where where) throws Exception {
        ...
        switch (where.logicOp) {
            ...
            case "and":
                ...
                break;
            default:
                throw ErrorConstants.InvalidLogOpException;
        }
        return res;
    }
