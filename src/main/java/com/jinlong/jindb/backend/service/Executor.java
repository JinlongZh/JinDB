...
import com.jinlong.jindb.backend.parser.Parser;
import com.jinlong.jindb.backend.parser.statement.*;
import com.jinlong.jindb.backend.tbm.BeginRes;
...
public class Executor {
    ...
    public byte[] execute(byte[] sql) throws Exception {
        ...
        if (Begin.class.isInstance(stat)) {
            if (xid != 0) {
                throw ErrorConstants.NestedTransactionException;
            }
            ...
            if (xid == 0) {
                throw ErrorConstants.NoTransactionException;
            }
